...
## Overview
This project implements a **machine learning model** to predict whether a patient has **diabetes** based on specific medical test results. The dataset used for training and testing comes from the Pima Indian Diabetes dataset.

## Dataset
The dataset is obtained from the following URL:
```
https://raw.githubusercontent.com/HackBio-Internship/2025_project_collection/refs/heads/main/Python/Dataset/diabetes.csv
```

The dataset consists of medical diagnostic features and a target variable that indicates whether the patient has diabetes (1) or not (0).
...
The dataset is loaded into a Pandas DataFrame using `pd.read_csv(url)`, and basic information about the dataset is displayed.

### 3. Data Exploration
- The first 5 rows of the dataset are displayed using `df.head()`.
- Dataset structure (column names, data types, and missing values) is checked using `df.info()`.
- Missing values are checked using `df.isnull().sum()`.
- A statistical summary is generated using `df.describe()`.

### 4. Data Preparation
- Features (`X`) and target (`y`) are defined.
- The dataset is split into **training (80%)** and **testing (20%)** using `train_test_split()`.
- Standardization is applied using `StandardScaler()` to normalize numerical values.

### 5. Model Training
A **Logistic Regression** model is trained using the training data.

### 6. Model Prediction
Predictions are made on the test dataset using `model.predict()`.

### 7. Model Evaluation
The model's accuracy is computed, and a **classification report** is generated to assess performance.

### 8. Saving the Model
The trained model is saved as `diabetes_prediction_model.pkl` using `joblib.dump()` for future use.
